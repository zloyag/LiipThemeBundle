...
/*
 * Overrided by zloyag for caching assetic blocks (stylesheets, javascripts) in twig templates by template|theme
 *
...
namespace Liip\ThemeBundle\Factory\Loader;
...
class CachedFormulaLoader implements FormulaLoaderInterface
{
     ...
     */
    public function __construct(FormulaLoaderInterface $loader, ConfigCache $configCache, ActiveTheme $activeTheme, $debug = false)
    {
    ...
    public function load(ResourceInterface $resources)
    {
        ...
        foreach ($resources as $resource) {
            $id = (string) $resource;
            $curTheme = $this->activeTheme->getName();
            ...
            $this->activeTheme->setName($curTheme);
        }
        return $formulae;
    }
}
